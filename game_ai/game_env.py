...
class Game(object):
    ...
    def can_kill(self, pos_x, pos_y):
        ...
        if self.is_out(up_pos_x, up_pos_y) and self.map[up_pos_x][up_pos_y] == 2:
            can_kill = True
        if self.is_out(down_pos_x, down_pos_y) and self.map[down_pos_x][down_pos_y] == 2:
            can_kill = True
        if self.is_out(left_pos_x, left_pos_y) and self.map[left_pos_x][left_pos_y] == 2:
            can_kill = True
        if self.is_out(right_pos_x, right_pos_y) and self.map[right_pos_x, right_pos_y] == 2:
            can_kill = True
